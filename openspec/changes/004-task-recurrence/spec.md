...
- **（2026-09-02 修订）** 归档只退出「活跃列表」：完成日当天 Today/任务墙仍以完成态留痕（completed 卡片可回看、可撤销），明天起退出；任务墙日期翻页可回看活动区间覆盖的历史日（含已归档任务，只读）。候选集由「区间覆盖当日」驱动，不再按 status 硬过滤。
- 撤销/减少作用于已归档一次性任务：undo 允许（补偿账本）；**（2026-09-02 修订）** 完成日当天 undo 到不再达标 → 自动解除当天归档（重开活动区间），任务回到待打卡；decrement 因归档被拒，跨天 undo 不解除归档，手动恢复后可继续。
