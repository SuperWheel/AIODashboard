...
describe("validateManifest（与 Rust validate 对齐）", () => {
  ...
  it("Rust 返回的 null quota 表示无存储权限，不阻止无存储插件加载", () => {
    expect(
      validateManifest({
        ...valid,
        api_version: "plugin.protocol/v2",
        permissions: { ui: ["command"], storage_quota_bytes: null },
        contributions: { commands: [{ id: "probe", title: "Probe" }] },
      }),
    ).toEqual([]);
  });
  ...
  it("扩展能力字段向后兼容并校验未知值", () => {
    expect(
      validateManifest({
        ...valid,
        api_version: "plugin.protocol/v1",
        permissions: {
          core: ["task.read"],
          ui: ["command"],
    ...
    expect(
      validateManifest({ ...valid, api_version: "plugin.protocol/v9" }).length,
    ).toBeGreaterThan(0);
    expect(
      validateManifest({ ...valid, permissions: { core: ["task.erase"] } })
        .length,
    ).toBeGreaterThan(0);
    expect(
      validateManifest({ ...valid, permissions: { storage_quota_bytes: 0 } })
        .length,
    ).toBeGreaterThan(0);
  });
  ...
  it("非法 host / 未知事件 / 错误 cron 拒绝", () => {
    ...
    expect(
      validateManifest({
        ...valid,
        permissions: { network: ["api.example.com:8443"] },
      }).length,
    ).toBeGreaterThan(0);
    expect(
      validateManifest({ ...valid, permissions: { events: ["task.deleted"] } })
        .length,
    ).toBeGreaterThan(0);
    expect(
...
describe("host 工具（与 Rust extract_host/validate_host 对齐）", () => {
  ...
  it("extractHost", () => {
    expect(extractHost("https://API.GitHub.com/repos/x?q=1")).toBe(
      "api.github.com",
    );
