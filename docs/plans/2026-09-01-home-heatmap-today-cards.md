...
- 主体 2026-09-01 完成并提交（4154542 / 533b8cb / 573976f，与 8/31 批按层混切）。
- 走查一轮修复（同日）：①HeatmapCell fluid 模式让周/月格子填满卡宽；②热力图统一不渲染未来日（右端=今天）+首屏滚到最右，纯前端过滤不动协议；③格内符号标记全删（纯色填充）；④首页热力图收窄 col-8 与今日任务对齐、右栏 row-span-2（右栏 DOM 先于今日任务才能跨行落位）；⑤今日任务改会话级位置快照（会话内不重排，刷新才沉底，废弃 3s 冻结窗口）。
- dev-log：2026-09-01 条目「走查修复」段。
