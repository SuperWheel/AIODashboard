...
## [2026-09-02] 任务页切换器规格统一

**需求简述**：任务页「进行中/已归档」「均衡混排/类型分区」选中色块与日期控件高度不一致，以日期色块为准统一。

**修复**：三个切换器统一为同一分段控件规格——`bg-hover p-0.5` 圆角容器 + `h-6 rounded-md` 内块，选中 = `bg-surface + shadow-sm`（与快速捕捉切换器同规格）；原先的文字按钮（`py-1` + accent/10 底色）下线。纯样式调整，check.sh 全绿。
